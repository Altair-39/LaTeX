...
\nt{Questi registri servono anche a trasportare dati di istruzioni non adiacenti.}
...
\begin{itemize}
  \item \textbf{Problemi Strutturali:} alcune combinazioni di istruzioni non possono essere eseguite simultaneamente (e.g. se si ha una sola ALU non la si può usare nello stesso ciclo di clock per fare più cose diverse);
...
\dfn{Stall}{
  ...
  \begin{itemize}
    \item le istruzioni avviate prima di I possono proseguire fino a essere completate;
    \item le istruzioni avviate dopo I devono essere fermate, fino a che non viene risolto il problema di I.
  \end{itemize}
}
...
\dfn{Problemi di Controllo}{
  I problemi di controllo sono dovuti al fatto che, nel caso di istruzioni di salto, il PC può cambiare. 
}
