...
\dfn{Generalized Linear Model (GLM)}{
...
\qs{}{Come possiamo risolvere dei problemi di classificazione utilizzando i modelli lineari?}

\begin{itemize}
	\item Possiamo cercare una funzione monotona differenziabile $g$ che collega l'output della regressione alle etichette di classificazione.
	\item Nello specifico vogliamo convertire l'output del modello lineare in etichette binarie (0/1). Si potrebbe utilizzare una funzione:
	      \[
		      y = f(z) = \begin{cases}
			      0,   & \text{if } z < 0 \\
			      0.5, & \text{if } z = 0 \\
		      ...
		      \end{cases}
	      \]
	\item Tuttavia questa funzione non è differenziabile.
	\item Per cui si usa la \fancyglitter{funzione logistica}:
	      \[y = f(z) = \frac{1}{1 + e^{-z}}.\]
	\item E la sua inversa:
	      \[z = f^{-1}(y) = \ln \frac{y}{1 - y}\]
	\item Usando questa funzione e la sua inversa si può modellare la probabilità che un certo input appartenga a una particolare classe. La probabilità della classe positiva:
	      \[
		      y' = p(y=1|\mathbf{x}) = g^{-1}(\mathbf{w}^{\top}\mathbf{x} + b) = \frac{1}{1 + e^{-(\mathbf{w}^{\top}\mathbf{x} + b)}}
	      \]
	\item Applicando la funzione di link a entrambi i membri otteniamo:
...
\nt{Si può fare lo stesso partendo dai log-odds.}

\dfn{Maximum Likelihood Estimation (MLE)}{
	L'idea è quella di trovare i parametri w e b che massimizzano la likelihood dei dati osservati. Si utilizza una distribuzione di Bernoulli\footnote{Usata negli esperimenti binari.}:
	\[
...
\begin{itemize}
	\item Per trovare i parametri ottimali possiamo lavorare con la log-likelihood. Assumendo $z_i = w^\top x_i + b$:
	      \begin{align*}
		      \ln p(y_i|\mathbf{x}_i) & = y_i \ln p(y_i = 1|\mathbf{x}_i) + (1-y_i) \ln p(y_i = 0|\mathbf{x}_i)    \\
		                              & = y_i \ln \frac{e^{z_i}}{1 + e^{z_i}} + (1-y_i) \ln \frac{1}{1 + e^{z_i}}  \\
		                              & = y_i \left( z_i - \ln(1 + e^{z_i}) \right) - (1-y_i) \ln(1 + e^{z_i})     \\
		                              & = y_i z_i - y_i \ln(1 + e^{z_i}) - \ln(1 + e^{z_i}) + y_i \ln(1 + e^{z_i}) \\
		                              & = y_i z_i - \ln(1 + e^{z_i})
	      \end{align*}
	\item Si vuole minimizzare la funzione costo:
	...
	\item Per un dataset con $n$ osservazioni indipendenti la likelihood totale è il prodotto delle likelihood individuali:
...
\paragraph{Idea principale:}

\begin{itemize}
	\item Si divide il problema in una serie di piccoli task di classificazione binaria.
	\item Per ogni task si crea un nuovo dataset e si addestra un semplice classificatore binario.
	\item Dopo di ché si combinano i risultati per fare la predizione finale.
\end{itemize}

\paragraph{Ci sono diverse alternative a questo approccio basico:}

\begin{itemize}
	\item One-vs-One (OVO\footnote{OwO}).
	\item One-vs-Rest (OvR).
	\item Many-vs-Many (MvM).
	\item Error-Correcting Output Codes (ECOC).
\end{itemize}

\dfn{Matrice di Codifica}{
	Una matrice di codifica è una matrice che definisce la relazione tra le classi originali ei classificatori binari.
}
\[
	\begin{array}{c|cccc}
		\hline
		    & h_1           & h_2           & h_3           & h_4           \\
		\hline
		C_1 & \color{blue}1 & \color{blue}1 & \color{red}-1 & \color{red}-1 \\
		C_2 & \color{blue}1 & \color{red}-1 & \color{blue}1 & \color{red}-1 \\
		C_3 & \color{red}-1 & \color{blue}1 & \color{red}-1 & \color{blue}1 \\
		C_4 & \color{red}-1 & \color{red}-1 & \color{blue}1 & \color{blue}1 \\
		\hline
	\end{array}
\]
...
\clm{}{}{
	\begin{itemize}
		\item Ogni colonna corrisponde a un classificatore binario che distingue tra due gruppi di classi, le entries nella matrice indicano quali classi devono essere considerate positive o negative per ciascun classificatore.
		\item La riga nella matrice di codifica che è più simile al vettore di predizione indica la \fancyglitter{classe predetta}.
	\end{itemize}
}
...
\ex{One-vs-One}{
	\[\begin{array}{c|cccccc}
			...
			\hline
			C_1 & \color{blue}1 & \color{blue}1 & \color{blue}1 & 0             & 0             & 0             \\
			C_2 & \color{red}-1 & 0             & 0             & \color{blue}1 & \color{blue}1 & 0             \\
			C_3 & 0             & \color{red}-1 & 0             & \color{red}-1 & 0             & \color{blue}1 \\
			C_4 & 0             & 0             & \color{red}-1 & 0             & \color{red}-1 & \color{red}-1 \\
			\hline
		\end{array}\]
}
...
\ex{One-vs-Rest}{
	\[\begin{array}{c|cccc}
			\hline
			    & h_1           & h_2           & h_3           & h_4           \\
			\hline
			C_1 & \color{blue}1 & \color{red}-1 & \color{red}-1 & \color{red}-1 \\
			C_2 & \color{red}-1 & \color{blue}1 & \color{red}-1 & \color{red}-1 \\
			C_3 & \color{red}-1 & \color{red}-1 & \color{blue}1 & \color{red}-1 \\
			C_4 & \color{red}-1 & \color{red}-1 & \color{red}-1 & \color{blue}1 \\
			\hline
		\end{array}
	\]}
...
\ex{Many-vs-Many}{
	\[\begin{array}{c|cccccc}
			...
			\hline
			C_1 & \color{blue}1 & \color{blue}1 & \color{red}-1 & \color{red}-1 & \color{red}-1 & \color{red}-1 \\
			C_2 & \color{blue}1 & \color{red}-1 & \color{red}-1 & \color{blue}1 & \color{blue}1 & \color{blue}1 \\
			C_3 & \color{blue}1 & \color{blue}1 & \color{blue}1 & \color{red}-1 & \color{blue}1 & \color{blue}1 \\
			C_4 & \color{red}-1 & \color{red}-1 & \color{blue}1 & \color{red}-1 & \color{blue}1 & \color{red}-1 \\
			\hline
		\end{array}
	\]
}

\nt{Sia OvO che OvR sono casi speciali di MvM.}

\dfn{Error-Correcting Output Codes}{
...
\clm{}{}{
	\begin{itemize}
		\item Trovare ECOC ottimale per un dato numero di classi è NP-Hard. 
		\item Utilizzare una matrice di codifica randomica può essere una buona euristica se il codice è abbastanza lunga. 
		\item Quando $k$ è piccolo l'efficacia è limitata.
	\end{itemize}
}

\paragraph{OvO vs OvR:}

\begin{itemize}
	\item OvO:
		\begin{itemize}
			\item Ogni classificatore è addestrato solo un sottoinsieme dei dati, quindi ha efficacia limitata se i dati sono scarsi.
			\item Può essere costosa quando ci sono tante classi.
		\end{itemize}
	\item OvR: 
		\begin{itemize}
			\item Produce sottoproblemi binari molto sbilanciati, anche se il dataset originale era bilanciato. 
			\item Usa l'intero dataset per ogni classificatore binario, ciò aumenta l'efficienza.
		\end{itemize}
\end{itemize}

\subsection{Problemi di Sbilanciamento}

Assumiamo di avere un dataset molto sbilanciato dove la classe di maggioranza ha 998 istanze e la classe di minoranza ne ha solo 2. Un classificatore può ottenere 99.8\% di accuratezza predicendo per tutte le istanze quelle di maggioranza, però ciò fallisce nell'identificare la classe di minoranza che spesso è la \fancyglitter{classe di interesse}.


\dfn{Threshold Update}{
	Assumendo di usare un classificatore lineare $y = \mathbf{w}^{\top} \mathbf{x} + b$ e che le classi con $y > 0.5$ vengono classificate positivamente, se abbiamo $m^+$ istanze positive e $m^-$ istanze negative possiamo utilizzare un likelihood ratio:
...
\clm{}{}{
	\begin{itemize}
		\item Il metodo più semplice consiste nel duplicare casualmente gli esempi nella classe di minoranza. 
		\item Può portare all'overfitting.
		\item \fancyglitter{SMOTE} (Synthetic Minority Oversampling Technique): si prendono esempi a coppie e si interpolano a vicenda prendendo i punti nel mezzo. 
		\item Ciò introduce un bias.
	\end{itemize}
}
...
\clm{}{}{
	\begin{itemize}
		\item Si rischia di perdere informazione (problematico per piccoli datasets). 
		\item \fancyglitter{EasyEnsemble:} si inducono più classificatori e si prende il voto di maggioranza.
	\end{itemize}
}
