...
\paragraph{Ragionamento su classi:}

\subsection{Introduzione}

\begin{itemize}
	\item La gerarchia delle classi
	      asserita mediante gli
	      assiomi di sottoclasse
	      può essere diversa da
	      quella inferita.
	\item Il reasoner può inferire
	      delle relazioni di
...
\nt{Il reasoner ha sempre ragione.}

\paragraph{Inserimento di individui:}

\begin{itemize}
	\item Gli individui vengono inseriti direttamente
	      in una classe.
	\item Dopo averli inseriti è possibile predicarne
	      le caratteristiche, cioè specificarne le
	      proprietà che li mettono in relazione con
	      altri individui o con un dato (class e
	      property assertions).
	\item Il ragionamento può eventualmente
	      ricollocare un certo individuo in un’altra
	      classe.
	\item Inserendo un individuo con le
	      caratteristiche di una certa classe,
	      l’individuo viene classificato come
	      appartenente alla classe dal reasoner.
	\item Anche se è stato collocato manualmente
	      in una classe più generale.
\end{itemize}

\dfn{Assunzione di Mondo Aperto}{
	Le inferenze effettuate dal reasoner possono
	sembrare poco intuitive in alcuni casi. Il ragionamento sulle ontologie OWL segue
	l’assunzione di mondo aperto:

	\begin{itemize}
		\item l fatto che un’informazione non sia
		      rappresentata nel sistema non determina
		      che essa sia assunta falsa.
	\end{itemize}
}

\cor{Quantificatore Universale (Only)}{
	Se un individuo viene collocato in due o più classi che hanno un Quantificatore only il reasoner rileva un'inconsistenza.
}

\cor{Classe Primitiva}{
	La classe primitiva può avere condizioni
	necessarie associate a essa tramite lo slot SubClassOf.
}

\cor{Classe Definita}{
	La classe definita è una classe equivalente a
	un insieme di condizioni necessarie e
	sufficienti, specificate mediante EquivalentTo.
}

\clm{}{}{
	\begin{itemize}
		\item Only non significa almeno uno.
		\item Il quantificatore universale si applica a tutti.
	\end{itemize}
}

\paragraph{Ragionamento su data properties:}

\begin{itemize}
	\item I valori assegnati a un individuo per
	      una certa data property devono
	      appartenere al data type
	      specificato come range.
	\item Se il datatype è diverso da quello
	      previsto, il reasoner rileva
	      un’inconsistenza.
\end{itemize}

\paragraph{Restrizioni su data properties:}

\begin{itemize}
	\item È possibile porre una restrizione su una
	      data property.
	\item Usando quantificatori e cardinalità
	      tuttavia si esprime un vincolo sul
	      numero delle relazioni, non sul valore
	      che assume la proprietà.
	\item Per creare classi definite in molti casi si
	      usano i pattern che individuano un range
	      specifico di valori rispetto al tipo di dato.
	\item Per esempio le stringhe che
	      cominciano con una certa lettera, gli
	      interi superiori a un certo valore, ecc\dots
\end{itemize}

\paragraph{Classi primitive e ragionamento:}

\begin{itemize}
	\item Le classi primitive non permettono ai reasoner di collocare gli individui nelle classi in modo automatico.
	\item Esse permettono ai reasoner di individuare inconsistenze con le asserzioni sugli individui.
\end{itemize}

\subsection{Esportare le Inferenze su Protege}

\begin{itemize}
	\item Protege contiene un tool per esportare le inferenze effettuate dal reasoner attivo in una nuova ontologia.
	\item È utile se si vogliono utilizzare le inferenze in un ambiente in cui non è disponibile un reasoner.
	\item Vengono esportate sia le triple che costituiscono l'ontologia sia le triple che rappresentano le inferenze.
\end{itemize}

\paragraph{Passi per l'esportazione:}

\begin{enumerate}
	\item Selezione del tipo di inferenze che si intendono esportare.
	\item Selezione di elementi aggiuntivi:
	      \begin{itemize}
		      \item Annotazioni.
		      \item Asserzioni contenute nell'ontologia.
	      \end{itemize}
	\item Inserimento dell'IRI della nuova ontologia.
	\item Percorso e file in cui salvare la nuova ontologia.
	\item Selezione della codifica.
\end{enumerate}

\paragraph{Inferenze esportate:}

\begin{itemize}
	\item Il set di inferenze di base non esporta tutte le
	      inferenze che sono visualizzate nell’editor.
	\item  Le inferenze rappresentate come triple nella
	      nuova ontologia si dicono materializzate.
	\item Nei casi reali, il trade-off tra tempo di
	      esportazione (e lo spazio occupato) e livello di
	      dettaglio delle inferenze dipende dall’utilizzo
	      che si intende fare dell’ontologia esportata.
\end{itemize}

\cor{Object Property Characteristics}{
	Il flag \fancyglitter{Object} (o data) Property Characteristics esporta le inferenze
	fatte sulle caratteristiche delle object properties.
}

\cor{Class Assertion (Individual Types)}{
	Il flag \fancyglitter{Individual types} esporta le inferenze sugli individui basate sulla
	gerarchia delle classi.
}

\cor{Property Assertions}{
	Il flag \fancyglitter{Property assertions} esporta le inferenze sulle proprietà
	degli individui.
}

\subsection{Serializzazione di OWL}

\paragraph{Dall'ontologia astratta alla sua rappresentazion:}

\begin{itemize}
	\item L’ovale al centro rappresenta la nozione astratta di
	      ontologia, che può essere pensata sia come struttura
	      astratta sia come grafo RDF.
	\item La sintassi di OWL nelle specifiche è descritta come
	      sintassi \fancyglitter{astratta}.
	\item L’ontologia può essere serializzata in varie sintassi.
	\item La semantica dell’ontologia (parte in basso) può essere
	      specificata direttamente (Direct Mapping, semantica
	      model-teoretica) oppure a seguito della conversione in
	      un grafo RDF (RDF-based semantics).
\end{itemize}

\dfn{Mapping verso RDF}{
	Il mapping di OWL verso RDF consiste nel rappresentare
	l’ontologia come triple RDF. È specificata come un insieme di mapping, dove l’operatore
	T mappa un’ontologia OWL O in un grafo RDF T(O).
}

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{02/owls.png}
	\caption{Specifiche di OWL.}
\end{figure}

\paragraph{OWL2 vs. OWL1:}

\begin{itemize}
	\item Property chains.
	\item Datatypes più ricchi e data ranges.
	\item Restrizioni sulla cardinalità.
	\item Proprietà asimmetriche, riflessive e disgiuntive.
	\item Più capacità di annotazione.
	\item Chiavi.
\end{itemize}

\paragraph{Profili di OWL2:}

\begin{itemize}
	\item OWL2 EL: progettato per grandi ontologies. Limitazioni:
	      \begin{itemize}
		      \item No negazione e disgiunzione, no quantificazione universale sulle proprietà.
		      \item No proprietà inverse.
	      \end{itemize}
	\item OWL QL (Query Language):  concepito per l’integrazione con le basi di
	      dati relazionali. Limitazioni:
	      \begin{itemize}
		      \item No quantificazione esistenziale in una class expression.
		      \item No property chains.
	      \end{itemize}
	\item OWL RL (Rule Language): concepito per applicazioni che richiedono
	      l’utilizzo del ragionamento automatico senza sacrificare troppa
	      espressività.
\end{itemize}
